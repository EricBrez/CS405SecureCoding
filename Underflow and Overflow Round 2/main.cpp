...
#include <stdexcept>    //std::overflow_error & underflow_error
...
T add_numbers(T const& start, T const& increment, unsigned long int const& steps)
{
    ...
    /*
    For every step
    Check for overflow of the result by comparing it to the numerical maximum limit of the data type minus the increment value
    If overflow is detected, throw exception
    Otherwise, perfrom the operation as normal
    */
    for (unsigned long int i = 0; i < steps; ++i)
    {
        if (result > std::numeric_limits<T>::max() - increment) {
            throw std::overflow_error("Error: Overflow occurred, returning.");
        }
        else {
            result += increment;
        }
    ...
    //Return the final result if no exception is thrown during calculation
    return result;
}
...
T subtract_numbers(T const& start, T const& decrement, unsigned long int const& steps)
{
    ...
    /*
    For every step
    Check for underflow by comparing the result to the sum of the numerical minimum limit of the data type and the decrement
    If underflow is detected, throw exception
    Otherwise, perform the operation as normal
    */
    for (unsigned long int i = 0; i < steps; ++i)
    {
        if (result < std::numeric_limits<T>::min() + decrement) {
            throw std::underflow_error("Error: Underflow occurred, returning.");
        }
        else {
            result -= decrement;
        }
...
void test_overflow()
{
    ...
    //Attempt to run the add_numbers() function for the given datatype (paramater values calculated elsewhere) and print the result
    try {
    ...
    Return to stop the operation from continuing with the error
    */
...
void test_underflow()
{
    ...
    //Attempt to run the subtract_numbers() function for the given datatype (paramater values calculated elsewhere) and print the result
    try {
